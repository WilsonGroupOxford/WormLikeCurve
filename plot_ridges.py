...
def fit_line(x_values, y_values):
    ...
    _slope, _intercept, _, _, _ = scipy.stats.linregress(x_values,
                                                         y_values)
...
def find_nearest_points(x_values, y_values,
                        slope: float,
                        intercept: float):
    """
...
def plot_line(xs,
    ...
    """
    this_plot = ax.scatter(xs, -ys,
                           marker="x",)
    if plot_fit:
        slope, intercept, rvalue = fit_line(xs, ys)
        nearest_xs, nearest_ys = find_nearest_points(xs,
                                                     -ys,
                                                     slope,
                                                     intercept)
        ax.scatter(nearest_xs, -nearest_ys,
                   color=this_plot.get_edgecolor())

    if junction_points is not None:
        ax.scatter(xs[junction_points], -ys[junction_points],
                   marker="s", color=this_plot.get_edgecolor(), s=80)
...
def split_line(xs, ys):
    ...
    for midpoint in range(2, np.shape(xs)[0]-1):
        ...
        # size?
        for split_xs, split_ys in [(xs[midpoint:], ys[midpoint:]),
                                   (xs[:midpoint], ys[:midpoint])]:
            split_slope, split_intercept, res_var = fit_line(split_xs,
                                                             split_ys)
    ...
    if min(split_rvalues) < rvalue:
        ...
        return [(xs[split_index:], ys[split_index:]),
...
if __name__ == "__main__":
    COORDS = np.genfromtxt("./Data/Results.csv",
                           delimiter=",",
                           skip_header=1,
                           usecols=[4, 5],
                           dtype=float)
    JUNC_TYPES = np.genfromtxt("./Data/Results.csv",
                               delimiter=",",
                               skip_header=1,
                               usecols=[7],
                               dtype=str)
    ...
    CONTOUR_IDS = np.genfromtxt("./Data/Results.csv",
                                delimiter=",",
                                skip_header=1,
                                usecols=[2],
                                dtype=int)
    ...
    LJ_PAIRS = clustering.find_lj_pairs(COORDS[ALL_JUNCTIONS_FLAT],
                                        ALL_JUNCTIONS_FLAT,
                                        4.0)
    LJ_CLUSTERS = clustering.find_lj_clusters(LJ_PAIRS)
    CLUSTER_POSITIONS = clustering.find_cluster_centres(LJ_CLUSTERS,
                                                        COORDS,
                                                        offset=0)
