...
from rings.ring_finder import (
    convert_to_ring_graph,
    topological_rdf,
    RingFinderError,
    geometric_rdf,
)
...
def find_powerlaw_fit(xs, ys):
    ...
    valid_entries = np.logical_and(
        np.logical_and(~np.isnan(ln_xs), ~np.isinf(ln_xs)),
        np.logical_and(~np.isnan(ln_ys), ~np.isinf(ln_ys)),
    )
        ...
        if len(selected) < 1:
            ...
            while O:
                ...
                if not newweight == weight:
                    break
                heapq.heappush(selected, [-degrees_left[i], -degrees_left[j], (i, j)])
        ...
        if a_list[i] and j in b_set:
            ...
            if not degrees_left[j] == 0:
                ...
                for k in b_set:
                    ...
                    if weight == w and not newweight > weight:
                        heapq.heappush(
                            selected, [-degrees_left[j], -degrees_left[k], (j, k)]
                        )
                    else:
            ...
            if not degrees_left[i] < 0 and not degrees_left[j] < 0:
                ...
                while selected2:
                    ...
                    if degrees_left[i] * degrees_left[j] > 0:
                        heapq.heappush(
                            selected, [-degrees_left[i], -degrees_left[j], (i, j)]
                        )
    return A_graph
...
def generate_ring_size_fig(ring_sizes, angle, length):
    ...
    rsax.errorbar(
        x=ring_size_idx,
        y=mean_ring_sizes,
        yerr=np.std(ring_size_arr, axis=0, ddof=1) / np.sqrt(ring_size_arr.shape[1]),
        capsize=5.0,
    )
    rs_smooth = np.linspace(0, ring_size_arr.shape[0], 50)
    rs_powerlaw = np.exp(intercept) * np.linspace(0, ring_size_arr.shape[0], 50) ** (
        grad
    )
...
def generate_node_degree_fig(node_degrees, angle, length):
    ...
    error_node_degrees = np.std(node_degrees_arr, axis=0, ddof=1) / np.sqrt(
        node_degrees_arr.shape[1]
    )
    ...
    ndax.errorbar(
        x=node_degrees_idx, y=mean_node_degrees, yerr=error_node_degrees, capsize=5.0
    )
    nd_smooth = np.linspace(0, node_degrees_arr.shape[0], 50)
    nd_powerlaw = np.exp(intercept) * np.linspace(0, node_degrees_arr.shape[0], 50) ** (
        grad
    )
        ...
        for LENGTH in ["0.01", "0.05", "0.1", "0.5", "1", "5", "10", "100"]:
            ...
            for REPETITION in REPETITIONS:
                ...
                for SUBDIR in os.scandir(DIRNAME):
                    ...
                    try:
                        ...
                        continue
                    try:
                        PRF = PeriodicRingFinder(
                            graph=GRAPH,
                            coords_dict=POS_DICT,
                            cell=BOX[:, 1],
                            missing_policy="return",
                        )
                        DUAL_CNX = nx.get_node_attributes(
                            DUAL_GRAPH, "dual_connections"
                        )
                        NEW_SHAPES = set()
                        ...
                        for value in DUAL_CNX.values():
                            ...
                            while True:
                                neighbours = [
                                    item for item in GRAPH.neighbors(current_node)
                                ]
                                neighbours_in_ring = [
                                    neighbour
                                    for neighbour in neighbours
                                    if neighbour in value
                                    and neighbour not in seen_nodes
                                ]
                                if not neighbours_in_ring:
                                    if start_node in neighbours:
                                        edges.append(
                                            frozenset([current_node, start_node])
                                        )
                                    break
                            ...
                            shape = Shape(
                                edges, coords_dict=POS_DICT, is_self_interacting=True
                            )
                    ...
                    print(
                        f"Saving figure to ./Results/Networks/{ANGLE}_{LENGTH}_{REPETITION}.pdf"
                    )
                    FIG.savefig(f"./Results/Networks/{ANGLE}_{LENGTH}_{REPETITION}.pdf")
                    plt.close(FIG)
                    THIS_RING_SIZES = normalise_counter(
                        Counter([len(ring) for ring in PRF.current_rings])
                    )
                    ...
                    THIS_NODE_DEGREES = normalise_counter(
                        Counter([degree for node_id, degree in GRAPH.degree()])
                    )
                    ...
                    ASSORTATIVITIES.append(
                        nx.numeric_assortativity_coefficient(RING_GRAPH, "size")
                    )
                    NODE_ASSORTATIVITIES.append(
                    ...
                    NODE_S_METRICS.append(
                        nx.s_metric(GRAPH, normalized=False) / NODE_SMAX
                    )

            generate_ring_size_fig(RING_SIZES, ANGLE, LENGTH)
            generate_node_degree_fig(NODE_DEGREES, ANGLE, LENGTH)

            TOTAL_ASSORTATIVITIES[(LENGTH, ANGLE)] = (
                np.mean(ASSORTATIVITIES),
                np.std(ASSORTATIVITIES, ddof=1),
            )
            TOTAL_NONRANDOMNESS[(LENGTH, ANGLE)] = (
                np.nanmean(NON_RANDOMNESS),
                np.nanstd(NON_RANDOMNESS, ddof=1),
            )
            TOTAL_SMETRICS[(LENGTH, ANGLE)] = (
                np.mean(S_METRICS),
                np.std(S_METRICS, ddof=1),
            )

            TOTAL_NODE_ASSORTATIVITIES[(LENGTH, ANGLE)] = (
                np.mean(NODE_ASSORTATIVITIES),
                np.std(NODE_ASSORTATIVITIES, ddof=1),
            )
            TOTAL_NODE_SMETRICS[(LENGTH, ANGLE)] = (
                np.mean(NODE_S_METRICS),
                np.std(NODE_S_METRICS, ddof=1),
            )
            ...
            for RINGSIZE in TOTAL_RDF.keys():
                ...
                for i, item in enumerate(TOTAL_RDF[RINGSIZE]):
                    if len(item) < max_data_size:
                        TOTAL_RDF[RINGSIZE][i] = np.pad(
                            item, [0, max_data_size - len(item)]
                        )
                ...
                NEWAX.fill_between(
                    [i for i in range(len(MEAN))], MEAN - STDEV, MEAN + STDEV, alpha=0.5
                )
            NEWAX.set_xlim(0, 10)
            NEWAX.set_xlabel("Topological Distance")
            NEWAX.plot(
                [0, 10], [6, 6], linestyle="dashed", color="black", label="Average"
            )
            NEWAX.set_ylim(0, 8)
            ...
            for RINGSIZE in TOTAL_GEOM_RDF.keys():
                ...
                for i, item in enumerate(TOTAL_GEOM_RDF[RINGSIZE]):
                    if len(item) < max_data_size:
                        TOTAL_GEOM_RDF[RINGSIZE][i] = np.pad(
                            item, [0, max_data_size - len(item)]
                        )
            ...
            NEWAX.plot(
                [0, 20], [6, 6], linestyle="dashed", color="black", label="Average"
            )
            NEWAX.set_ylim(0, 8)
            NEWAX.set_ylabel("Average ring size at distance")
            NEWAX.legend()
